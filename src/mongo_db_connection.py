...
from src.constants import DATABASE_NAME, MONGODB_URL_KEY, MONGODB_USER_KEY, MONGODB_PASSWORD_KEY
    ...
    def __init__(self, database_name: str = DATABASE_NAME) -> None:
        ...
        try:
            ...
            if MongoDBClient.client is None:
                mongo_db_uri = os.getenv(MONGODB_URL_KEY)  # Retrieve MongoDB URL from environment variables
                mongo_user = os.getenv(MONGODB_USER_KEY)
                mongo_pwd = os.getenv(MONGODB_PASSWORD_KEY)
                mongo_db_url = mongo_db_uri.replace('<username>',mongo_user)
                mongo_db_url = mongo_db_url.replace('<db_password>',mongo_pwd)
